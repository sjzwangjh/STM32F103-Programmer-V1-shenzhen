...
typedef struct{     // STK鍙傛暟缁?瀛楄妭鏁扮粍杞崲杈呭姪鏋勪綋
    uint8_t     bytes[32];
    struct{
        int     buildVersionLow;
...
typedef struct stkEnterProgIcsp{
    uint8_t   deviceProfile;    /* 鍏煎鏃у崗璁? 0=娌跨敤褰撳墠, 1=baseline, 2=mid-range, 3=enhanced */
...
typedef struct stkProgramFlashIcsp{
    ...
    uint8_t   flags;
    uint8_t   delay;
    uint8_t   data[1];          /* 瀹為檯鍚庨潰璺熼殢鏇村鏁版嵁 */
...
#define STK_DEVICE_ITEM_ID_LEN                  6
#define STK_DEVICE_ITEM_DESC_LEN                64
...
/* 鑷坊鍔犲崗璁?> 鍣ㄤ欢鏍囪瘑淇℃伅缁撴瀯浣?姝ょ粨鏋勪綋涓嶅彲杞绘槗淇敼 */
typedef struct
{
